...
import { ItemMacroOptions } from '../systems/dnd5e/itemMacroOptions.js'
...
function registerColorSettings (appName, module) {
    ...
    Hooks.once('ready', () => {
        document
            ...
            .style.setProperty(
                '--tah-background',
                game.settings.get(appName, 'background') ?? '#00000000'
            )
        document
            .querySelector(':root')
            .style.setProperty(
                '--tah-button',
                game.settings.get(appName, 'buttonBackgroundColor') ?? '#00000080'
            )
        document
            .querySelector(':root')
            .style.setProperty(
                '--tah-button-outline',
                game.settings.get(appName, 'buttonBorderColor') ?? '#000000ff'
            )
    })
}
