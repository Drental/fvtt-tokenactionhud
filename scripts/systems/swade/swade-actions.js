...
export class ActionHandlerSwade extends ActionHandler {
    ...
    _addAttributes (list, tokenId, actor) {
        ...
        Object.entries(attr).forEach((a) => {
            ...
            let name
            if (settings.get('abbreviateAttributes')) { name = this.i18n(nameData.short) } else name = this.i18n(nameData.long)

            const encodedValue = [actionType, tokenId, key].join(this.delimiter)
            const action = { name, encodedValue, id: key }
        ...
        const cat = this.initializeEmptyCategory('attributes')
    ...
    _addSkills (list, tokenId, actor) {
        ...
        skills.forEach((s) => {
            ...
            const action = { name: s.name, encodedValue, id: s.id }

            const mod = this._parseDie(s.system.die, s.system['wild-die'])
            action.info1 = mod
    ...
    _groupPowers (powers) {
        ...
        return powerTypes.reduce((grouped, p) => {
            ...
            if (!Object.hasOwn(grouped, p)) grouped[powerName] = []
    ...
    _addWoundsAndFatigue (list, tokenId, actor) {
        ...
        const woundsCategory = this.initializeEmptyCategory('wounds')
    ...
    _addStatuses (list, tokenId, actor) {
        ...
        Object.entries(statuses).forEach((s) => {
            ...
            const action = { name, id: name, encodedValue }
    ...
    _addUtilities (list, tokenId, actor) {
        ...
        const cat = this.initializeEmptyCategory('utility')
    ...
    /** @private */
    _parseDie (die, wild) {
        const dieMod = this._buildDieString(die)

        if (!wild) return dieMod

        const wildMod = this._buildDieString(wild)
        if (
    ...
    _buildItemAction (tokenId, item) {
        ...
        const encodedValue = [actionType, tokenId, id].join(this.delimiter)
        const action = { name, id, encodedValue }
