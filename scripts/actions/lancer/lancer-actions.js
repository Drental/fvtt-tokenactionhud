...
export class ActionHandlerLancer extends ActionHandler {
    ...
    async doBuildActionList(token, multipleTokens) {
        ...
        switch (actor.data.type) {
            case 'pilot':
                this._combineCategoryWithList(result, this.i18n('tokenactionhud.pilot'), this._pilotCategory(mm, actorId));
                break;
            case 'mech':
                this._combineCategoryWithList(result, this.i18n('tokenactionhud.pilot'), this._pilotCategory(mm.Pilot, mm.Pilot.RegistryID));
                this._combineCategoryWithList(result, this.i18n('tokenactionhud.mech'), this._mechCategory(mm, actorId));
                this._combineCategoryWithList(result, this.i18n('tokenactionhud.weapons'), this._weaponsCategory(mm.Loadout.WepMounts, actorId));
                this._combineCategoryWithList(result, this.i18n('tokenactionhud.systems'), this._systemsCategory(mm.Loadout.SysMounts, actorId));
                break;
            case 'npc':
                this._combineCategoryWithList(result, this.i18n('tokenactionhud.stats'), this._npcBaseCategory(actor, actorId));
                this._combineCategoryWithList(result, this.i18n('tokenactionhud.features'), this._npcFeatureCategory(actor, actorId));
                break;
        }
    ...
    _makeAction(actionName, macroType, actorId, actionId, option) {
        ...
        action.encodedValue = [macroType, actorId, actionId, JSON.stringify(option ? option : {})].join(this.delimiter);
        return action;
    }
    ...
    _npcWeaponSubCat(actor, actorId) {
        return this._makeNPCItemSubCat(this.i18n('tokenactionhud.weapons'), 'Weapon', actor, actorId)
    }

    _npcTraitSubCat(actor, actorId) {
        return this._makeNPCItemSubCat(this.i18n('tokenactionhud.traits'), 'Trait', actor, actorId)
    }

    _npcSystemSubCat(actor, actorId) {
        return this._makeNPCItemSubCat(this.i18n('tokenactionhud.systems'), 'System', actor, actorId)
    }

    _npcTechSubCat(actor, actorId) {
        return this._makeNPCItemSubCat(this.i18n('tokenactionhud.techs'), 'Tech', actor, actorId)
    }

    _npcReactionSubCat(actor, actorId) {
        return this._makeNPCItemSubCat(this.i18n('tokenactionhud.reactions'), 'Reaction', actor, actorId)
    }

    _skillsSubCategory(mm, actorId) {
        return this._makeItemSubCat(this.i18n('tokenactionhud.skilltriggers'), 'skill', mm._skills, actorId)
    }
    ...
    _talentsSubCategory(mm, actorId) {
        ...
        let itemSubCats = mm._talents.filter(item => {
    ...
    _pilotWeaponSubCategory(mm, actorId) {
        return this._makeItemSubCat(this.i18n('tokenactionhud.weapons'), 'pilot_weapon', mm.Loadout.Weapons, actorId)
    }

    _pilotGearSubCategory(mm, actorId) {
        return this._makeItemSubCat(this.i18n('tokenactionhud.gear'), 'pilot_gear', mm.Loadout.Gear, actorId)
    }
    ...
    _coreBonSubCategory(mm, actorId) {
        ...
        result.actions = corebonus.map(bonus => {
          ...
          return this._makeAction(bonus.Name, 'coreBonus',  actorId, bonus.RegistryID, option);
        });
        return result;
    }
    ...
    _corePowerSubCategory(frame, actorId) {
        ...
        if(core.PassiveName != "Core Passive") {
            result.actions.push( this._makeAction(core.PassiveName, 'corePassive', actorId, '') )
        }
        if(core.ActiveName) {
            result.actions.push( this._makeAction(core.ActiveName, 'coreActive', actorId, '') )
        }
    ...
    _weaponsCategory(WepMounts, actorId) {
        ...
        let itemSubCats = WepMounts.map((mount, i) => {
            ...
            subcat.actions = mount.Slots.filter(slot => slot.Weapon !== null).map(slot => {
                return this._makeAction(slot.Weapon.Name, macro, actorId, slot.Weapon.RegistryID)
            });
    ...
    _systemsCategory(loadout, actorId) {
        ...
        let itemSubCats = loadout.flatMap(mount => mount.System).map(system => {
            ...
            let activations = system.Actions.map((action,i) => {
              ...
              option.Type = "Action";
              option.Index = i;
              return this._makeAction(action.Name, 'activation', actorId, system.RegistryID, option);
            });
            ...
            let deployables = system.Deployables.map((deployable, i) => {
              ...
              option.Index = i;
              return this._makeAction(deployable.Name, 'activation', actorId, system.RegistryID, option);
            });
